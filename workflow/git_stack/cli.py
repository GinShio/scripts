...
def main():
    ...
    create_parser = subparsers.add_parser(
        "create",
        help="[Deprecated] Create or update PRs for the stack (Use sync --pr --no-push)",
    )
    ...
    anno_parser.add_argument(
        "--all",
        action="store_true",
        help="Annotate ALL stacks in .git/machete (default: current stack only)",
    )
    ...
    if args.command == "slice":
        ...
        if not args.all:
            ...
            if curr:
                ...
                print(
                    "Warning: Could not detect current branch. Annotating all stacks."
                )
