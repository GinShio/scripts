//! `wf stack submit` — make the MRs match the stack.
//!
...
pub fn run(repo: &Repository, args: &SubmitArgs) -> anyhow::Result<()> {
    ...
    let fix_results = map_parallel(&fixes, |(branch, base, id, display)| {
        if wf_log::is_dry_run() {
            wf_log::dry_run(&format!("retarget {noun} {display} ({branch}) -> {base}"));
            return Ok(());
        }
    ...
    for (branch, base) in &creates {
        ...
        if wf_log::is_dry_run() {
            ...
            wf_log::dry_run(&format!("create {noun} for {branch} -> {base}{tag}"));
            continue;
        }
        let req = NewMr {
            branch: branch.clone(),
...
mod tests {
    use super::*;
    use crate::util::forge::{MergeRequest, MrState};
