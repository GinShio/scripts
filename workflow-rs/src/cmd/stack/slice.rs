//! `wf stack slice` — cut the commits on top of a base into named branches.
//!
...
use crate::core::git::{Commit, Repository};
...
pub fn run(repo: &Repository, base: Option<&str>) -> anyhow::Result<()> {
    ...
    let content_path = tmp.join(format!("wf-slice-{unique}.todo"));
    let capture_path = tmp.join(format!("wf-slice-{unique}.capture"));
    let script_path = tmp.join(format!("wf-slice-{unique}.sh"));
