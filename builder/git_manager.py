...
class GitManager:
    ...
    def update_repository(
        ...
        *,
        ...
        if not repo_path.exists():
            if clone_script:
                self._run_script(clone_script, repo_path)
            else:
                parent = repo_path.parent
                parent.mkdir(parents=True, exist_ok=True)
                self._runner.run(["git", "clone", url, str(repo_path), "--recursive"], cwd=parent)
            return

        if update_script:
            self._run_script(update_script, repo_path)
            return
        ...
        self._runner.run(["git", "fetch", "--all"], cwd=repo_path)
        result = self._runner.run(["git", "switch", main_branch], cwd=repo_path, check=False)
        if result.returncode != 0:
            self._runner.run(["git", "switch", "-c", main_branch, f"origin/{main_branch}"], cwd=repo_path)
        self._runner.run(["git", "submodule", "update", "--recursive"], cwd=repo_path)

        if component_branch:
            component_path = repo_path
            self._runner.run(["git", "fetch", "--all"], cwd=component_path)
            result = self._runner.run(["git", "switch", component_branch], cwd=component_path, check=False)
            if result.returncode != 0:
                raise RuntimeError(f"Component branch '{component_branch}' does not exist")
            self._runner.run(["git", "submodule", "update", "--recursive"], cwd=component_path)

        if stash_applied:
            self._runner.run(["git", "switch", main_branch], cwd=repo_path)
            self._runner.run(["git", "stash", "pop"], cwd=repo_path, check=False)
    ...
    def _run_script(self, script: str, cwd: Path) -> None:
